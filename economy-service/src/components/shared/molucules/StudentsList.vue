<template>
    <div>
        ...
        <table class="students-list">
            <tr v-for="({ similarity, student }, index) in students" :key="index" class="item">
                <td class="index">{{ index + 1 }}</td>
                <td class="name">{{ student.firstname }} {{ student.lastname }}</td>
                <td class="similarity" :class="{ lowSimilariry: Math.round(similarity) < 50 }">
...
.students-list .similarity, .students-list .index {
    text-align: center;
}
