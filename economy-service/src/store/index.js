...
export default new Vuex.Store({
  ...
  actions: {
    async sendGroupData(ctx, { groups, photos, discipline }) {
      ...
      fetch(
        `${process.env.VUE_APP_CORS_URL}${process.env.VUE_APP_BASE_URL}check`,
        {
          method: "POST",
          body: formData,
          headers: {
    ...
    loadDisciplines(ctx) {
      fetch(
        `${process.env.VUE_APP_CORS_URL}${process.env.VUE_APP_BASE_URL}disciplines`
      )
    ...
    loadGroups(ctx) {
      fetch(
        `${process.env.VUE_APP_CORS_URL}${process.env.VUE_APP_BASE_URL}groups`
      )
        .then((response) => response.json())
        .then((result) => {
